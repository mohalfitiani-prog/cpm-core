...
test('owner cannot create stage/item or submit a delivery',()=>{for(const a of ['stage','item','deliver'])assert.throws(()=>mutate(fixture(),'w',a,{}));});
test('contractor cannot approve own work or write NCR procedure',()=>{assert.throws(()=>mutate(fixture(),'c','reviewDelivery',{}));assert.throws(()=>mutate(fixture(),'c','reviewNcr',{}));});
test('stage and item limits are enforced',()=>{let p=fixture();p.stages=Array.from({length:10},()=>({id:'s',items:[]}));assert.throws(()=>mutate(p,'o','stage',{name:'a',description:'d',responsible:'c',due:'2026-10-01'}));p=fixture();p.stages[0].items=Array.from({length:5},()=>({id:'i'}));assert.throws(()=>mutate(p,'c','item',{stageId:'s',name:'a',description:'b',conditions:'c'}));});
test('RFI recipient alone can answer; contractor visibility is limited',()=>{let p=fixture();mutate(p,'w','rfi',{subject:'q',description:'q',recipient:'e'});assert.equal(projectView(p,'c').rfi.length,0);assert.throws(()=>mutate(p,'o','replyRfi',{id:p.rfi[0].id,reply:'a'}));mutate(p,'e','replyRfi',{id:p.rfi[0].id,reply:'a'});assert.throws(()=>mutate(p,'e','replyRfi',{id:p.rfi[0].id,reply:'b'}));});
test('pending files are private until approved',()=>{let p=fixture();mutate(p,'c','file',{name:'drawing',path:'projects/p/c/file',category:'plans'});assert.equal(projectView(p,'w').files.length,0);assert.equal(projectView(p,'c').files.length,1);mutate(p,'e','moderate',{collection:'files',id:p.files[0].id,status:'approved'});assert.equal(projectView(p,'w').files.length,1);});
